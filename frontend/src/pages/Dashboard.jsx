...
export default function Dashboard() {
  ...
  const handleExitAndReport = async () => {
    try {
      ...
      toast.dismiss();
      toast.success("Interview report sent to Admin Email successfully! 📩");
      setShowChat(false);
      setMessages([]);
    } catch {
      toast.dismiss();
    ...
    <div style={{ minHeight: "100vh", background: "#0A0A0F", color: "#fff", fontFamily: "'Segoe UI', system-ui, sans-serif" }}>
      ...
      <nav style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", borderBottom: "1px solid rgba(255,255,255,0.06)", background: "rgba(10,10,15,0.98)" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          <div style={{ fontSize: 14, color: "#999" }}>{user?.name || "User"}</div>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "32px 20px" }}>
        ...
        {showStudentPopup && (
          <div style={{ position: "fixed", top: 0, left: 0, width: "100vw", height: "100vh", background: "rgba(0,0,0,0.75)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 999 }}>
            <div style={{ background: "#13131A", padding: 30, borderRadius: 16, border: "1px solid rgba(255,255,255,0.1)", width: "100%", maxWidth: 380 }}>
              <h3 style={{ marginTop: 0, marginBottom: 6, fontSize: 18 }}>Student Metadata 📋</h3>
              <p style={{ color: "#777", fontSize: 13, marginBottom: 20 }}>Fill details to initialize institutional metrics tracking.</p>
              
              <label style={{ display: "block", fontSize: 13, color: "#999", marginBottom: 6 }}>Age</label>
              <input type="number" value={studentDetails.age} onChange={e => setStudentDetails({...studentDetails, age: e.target.value})} placeholder="e.g. 22" style={{ width: "100%", padding: "10px 14px", borderRadius: 8, background: "#0A0A0F", border: "1px solid rgba(255,255,255,0.1)", color: "#fff", fontSize: 14, marginBottom: 16, outline: "none", boxSizing: "border-box" }} />
              
              <label style={{ display: "block", fontSize: 13, color: "#999", marginBottom: 6 }}>Academic Branch</label>
              <input type="text" value={studentDetails.branch} onChange={e => setStudentDetails({...studentDetails, branch: e.target.value})} placeholder="e.g. CSE" style={{ width: "100%", padding: "10px 14px", borderRadius: 8, background: "#0A0A0F", border: "1px solid rgba(255,255,255,0.1)", color: "#fff", fontSize: 14, marginBottom: 24, outline: "none", boxSizing: "border-box" }} />
              
              <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
                <button onClick={() => setShowStudentPopup(false)} style={{ background: "transparent", border: "1px solid #333", color: "#aaa", padding: "8px 16px", borderRadius: 8, cursor: "pointer", fontSize: 13 }}>Cancel</button>
                <button onClick={startInterview} style={{ background: "#F59E0B", border: "none", color: "#000", padding: "8px 18px", borderRadius: 8, fontWeight: 600, cursor: "pointer", fontSize: 13 }}>Begin 🚀</button>
              </div>
            </div>
          </div>
        )}
        ...
        {activeTab === "interview" && showChat && (
          <div style={{ background: "#13131A", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 14, overflow: "hidden" }}>
            <div style={{ padding: "14px 20px", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <span style={{ fontSize: 14, color: "#999" }}>🎤 Mock Interview Session</span>
              <button onClick={handleExitAndReport} style={{ background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.2)", color: "#ef4444", padding: "5px 12px", borderRadius: 6, cursor: "pointer", fontSize: 12, fontWeight: 600 }}>End & Send Report 📩</button>
            </div>
            <div style={{ height: 340, overflowY: "auto", padding: 20, display: "flex", flexDirection: "column", gap: 12 }}>
              {messages.map((m, i) => (
                <div key={i} style={{ display: "flex", justifyContent: m.sender === "user" ? "flex-end" : "flex-start" }}>
                  <div style={{ maxWidth: "75%", padding: "10px 14px", borderRadius: 10, fontSize: 14, lineHeight: 1.5, background: m.sender === "user" ? "#F59E0B" : "#1a1a2a", color: m.sender === "user" ? "#000" : "#ccc" }}>{m.text}</div>
                </div>
              ))}
            ...
            <div style={{ padding: "12px 20px", borderTop: "1px solid rgba(255,255,255,0.06)", display: "flex", gap: 10 }}>
              <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && sendMessage()} placeholder="Type your answer and press Enter..." style={{ flex: 1, padding: "10px 14px", borderRadius: 8, background: "#0A0A0F", border: "1px solid rgba(255,255,255,0.1)", color: "#fff", fontSize: 14, outline: "none" }} />
              <button onClick={sendMessage} disabled={chatLoading} style={{ padding: "10px 16px", borderRadius: 8, border: "none", background: "#F59E0B", color: "#000", cursor: "pointer" }}>
                <Send size={16} />
              </button>
            </div>
          </div>
        ...
        {activeTab === "jd" && (
          <div style={{ background: "#13131A", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 14, padding: 24 }}>
            ...
            <textarea value={jdText} onChange={(e) => setJdText(e.target.value)} placeholder="Paste the job description here..." style={{ width: "100%", height: 150, padding: "12px 14px", borderRadius: 8, background: "#0A0A0F", border: "1px solid rgba(255,255,255,0.1)", color: "#fff", fontSize: 13, outline: "none", resize: "vertical", boxSizing: "border-box" }} />
            <button onClick={handleJDMatch} disabled={jdLoading} style={{ marginTop: 12, padding: "11px 28px", borderRadius: 8, border: "none", background: jdLoading ? "#333" : "#F59E0B", color: jdLoading ? "#666" : "#000", fontWeight: 700, cursor: jdLoading ? "not-allowed" : "pointer", fontSize: 14 }}>
        ...
        {activeTab === "cover" && (
          <div style={{ background: "#13131A", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 14, padding: 24 }}>
            ...
            <input value={companyName} onChange={(e) => setCompanyName(e.target.value)} placeholder="Company name (e.g. Google, Flipkart)" style={{ width: "100%", padding: "10px 14px", borderRadius: 8, background: "#0A0A0F", border: "1px solid rgba(255,255,255,0.1)", color: "#fff", fontSize: 14, outline: "none", boxSizing: "border-box", marginBottom: 12 }} />
            <button onClick={handleCoverLetter} disabled={coverLoading} style={{ padding: "11px 28px", borderRadius: 8, border: "none", background: coverLoading ? "#333" : "#F59E0B", color: coverLoading ? "#666" : "#000", fontWeight: 700, cursor: coverLoading ? "not-allowed" : "pointer", fontSize: 14 }}>
            ...
            {coverLetter && (
              <div style={{ marginTop: 20 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                  ...
                  <button onClick={() => { navigator.clipboard.writeText(coverLetter); toast.success("Copied!"); }} style={{ padding: "6px 14px", borderRadius: 8, border: "1px solid rgba(255,255,255,0.15)", background: "transparent", color: "#fff", cursor: "pointer", fontSize: 12 }}>📋 Copy</button>
                </div>
